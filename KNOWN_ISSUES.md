...
| 11 | **嵌入侧栏内 Ctrl/Cmd+Enter 把草稿直接提交**（上游 DSH `dffe955ed2`，2026-08-02，把输入框既定的"换行"组合键改成了加速提交/steer——在草稿上写多行回复按 Ctrl+Enter 换行时消息瞬间发出）/ **Ctrl/Cmd+Enter submits the composer draft in the embedded sidebar** (upstream repurposed the newline chord into accelerated submit/steer) | 修复于本仓库 [Unreleased]：`dsh-vscode-integration` 0.8.0 嵌入式 Ctrl/Cmd+Enter→换行桥（Lexical contenteditable 上遮蔽 shiftKey 放行、textarea 上经 execCommand 插入；空草稿放行保留上游 steer 手势；普通 Enter/Shift+Enter/输入框外不受影响；已在真实运行时 0.1.2-rc.1 实测）/ fixed here in [Unreleased]: runtime-integration 0.8.0 restores the chord embed-only (verified live against the real runtime) |
