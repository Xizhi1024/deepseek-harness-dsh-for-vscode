...
- **侧边栏 iframe 在认证运行时（≥0.1.2-rc.1）下永远渲染 DSH 的 401 文本**：rc.1 的浏览器认证把 SameSite=Strict cookie 绑定到正常浏览器标签页，而 webview 里的 DSH iframe 是第三方帧（顶层源为 `vscode-webview://`）——Chromium 对第三方上下文既不保存也不发送 Strict cookie，因此 token 交换即使成功，iframe 跟随 303 后仍然无凭据，401 响应体被原样渲染。端口隔离（F5 落 3200）只是消除了叠加其上的端口易主问题，并未触及这层。新增 `src/webEmbedProxy.js` 回环转发器：iframe 改指向扩展宿主持有的无凭据回环端口，代理在 Node 侧持有 `exchangeLaunchCookie`（新导出于 loopbackAuth）换来的 cookie 并逐请求附带，把 Host 改写为子进程 authority、Origin 改写为子进程源、剥离 Sec-Fetch-*（对应子进程 browser-trust fence 的三条规则），请求/响应字节透传（events.mux SSE 流式不缓冲）；上游 401 时后台重铸 cookie，子进程重启后的后续请求自愈。仅对 owned 且带 token 的 server 启用代理；无 token/复用实例保持直连。浏览器入口（"Open in browser"、`dsh.openInBrowser`、framePage 回退链接）始终使用子进程启动 URL，代理地址不出窗口；代理绑定回环临时端口、随窗口销毁、每次 rebind 重建。边界：该端口等效于一个可无凭据访问本窗口子进程的回环门（与子进程自身的预认证 index 同级信任），是止血层，长期方案仍是 harness 侧嵌入认证契约。
  The sidebar iframe always rendered DSH's 401 body on authenticated runtimes (>=0.1.2-rc.1): rc.1's browser auth binds a SameSite=Strict cookie to a normal browser tab, but inside a webview the DSH iframe is a third-party frame (top-level origin `vscode-webview://`) — Chromium neither stores nor sends Strict cookies there, so even a successful token exchange left the iframe credential-less after the 303. Port isolation (F5 on 3200) only removed the port-takeover layer on top. New `src/webEmbedProxy.js` loopback forwarder: the iframe now targets a credential-less loopback port held by the extension host; the proxy attaches the cookie exchanged host-side by the new loopbackAuth export `exchangeLaunchCookie`, rewrites Host to the child authority, Origin to the child origin, and strips Sec-Fetch-* (the child fence's three rules), piping request/response bytes untouched (events.mux SSE streams without buffering); an upstream 401 re-mints the cookie in the background so requests after a child restart self-heal. The proxy is only enabled for owned, token-carrying servers; tokenless/reused endpoints stay direct. Browser entry points ("Open in browser", `dsh.openInBrowser`, the framePage fallback link) always use the child launch URL — the proxy address never leaves the window; the proxy binds an ephemeral loopback port, dies with the window, and is rebuilt on every rebind. Boundary: that port is a loopback door to this window's child without credentials (same trust level as the child's own pre-auth index) — a containment measure, the long-term fix remains a harness-side embed auth contract.
- **嵌入 UI 加载成功后所有操作阻塞、工作区列表为空**：Typert 实时通道走 `ws://…/api/remote.mux` WebSocket，而代理初版只转发普通 HTTP（`Upgrade` 属于逐跳头被剥离），握手 404、客户端无限重连——REST 全通而实时订阅全断，界面渲染正常但工作区/会话状态永远空转。代理现以原始套接字双向转发 WebSocket upgrade（握手请求按护栏规则改写 Host/Origin，帧含 permessage-deflate 原样透传）。已在第三方 iframe 复现环境中端到端验证：工作区与会话树加载、模型选择器就位、控制台零错误。同时确认 F5 隔离家缺模型供应商配置会导致消息永久挂起——将真实 `~/.dsh/settings.yaml` 复制进 `.vscode-test/f5/dsh-home`（用户已确认），密钥经 apiKeyEnv 环境变量注入，F5 宿主需能看到 ZAI_API_KEY。
  After the proxy landed, the embedded UI loaded but every operation blocked and the workspace list stayed empty: the Typert live channel rides `ws://…/api/remote.mux`, and the proxy's first version only forwarded plain HTTP (the Upgrade header is hop-by-hop and was stripped) — the handshake got a 404 and the client retried forever, leaving REST calls fine while every live subscription spun empty. The proxy now relays WebSocket upgrades as raw bidirectional sockets (handshake rewritten per the fence rules, frames including permessage-deflate passed through untouched), verified end to end in a third-party iframe repro: workspace and session trees load, the model selector is populated, zero console errors. Separately confirmed: an F5 isolated home without model-provider config blocks prompts forever — the real `~/.dsh/settings.yaml` was copied into `.vscode-test/f5/dsh-home` (user-approved); keys ride apiKeyEnv environment variables, so the F5 host must see ZAI_API_KEY.

### Fixed / 修复（1.1.5 本地验证版本）

- Keep the required integration overlay after a failed boot; never report a bare web server as a recovered sidebar. 合并重复连接请求，自动失败重试冷却 30 秒，手动重试可立即执行。
- Complete the rc.1 launch-token / 303 / Cookie exchange for health, session/workspace requests and event streams; reuse the owned authenticated endpoint.
- Integration 0.9.1 restores workspace.list/create through workspaceController, consumes and closes one Workspace baseline stream, and supplies cancellation signals to session methods. Legacy apiProxy routing remains dynamically selected.
- Add an isolated real-runtime smoke command: `node scripts/smoke-runtime.js <absolute DSH package root>`; it checks binding, listing, renaming, event-stream connection and reuse without touching user sessions.

### Changed / 变更（F5 调试边界隔离）

- **F5 Extension Development Host 不再复用日常 VS Code 与正式安装版的状态边界**：`Run Extension` 把扩展目录、`DSH_HOME` 和扩展自有 storage root 固定到仓库内已忽略的 `.vscode-test/f5/` 子目录，同时禁用其他用户扩展与 Workspace Trust 提示。实测表明 extensionHost debug 对 `--user-data-dir` 的处理仍可能复用当前 VS Code 主进程，因此扩展不再把隔离寄托于该参数：`DSH_VSCODE_STORAGE_ROOT` 明确重定向 journal/snapshots、heartbeat、runtime、实例注册表及启动日志；`DSH_HOME` 独立 profile/插件树。DSH 可执行文件仍按正常 runtime 解析规则选择，以验证真实版本兼容行为。
  The F5 host keeps its extension directory, `DSH_HOME`, and extension-owned storage root under the git-ignored `.vscode-test/f5/` tree, with unrelated user extensions and Workspace Trust prompts disabled. Because a live extensionHost debug session proved that `--user-data-dir` may still reuse the current VS Code main process, isolation no longer relies on that flag: `DSH_VSCODE_STORAGE_ROOT` explicitly redirects journals/snapshots, heartbeats, runtime files, the instance registry, and spawn logs, while `DSH_HOME` owns a separate profile/plugin tree. Runtime executable resolution remains production-equivalent for real compatibility coverage.
- **F5 端口隔离：`DSH_VSCODE_PORT` 固定开发宿主默认端口（launch.json 现为 3200）**：此前 F5 子进程与正式窗口共用默认端口 3080，任何一次端口易主都会让仍持有旧启动 token / 旧家目录 cookie 的侧边栏 iframe 直接渲染 DSH 的 401 文本（F5 隔离家与共享家的 cookie 签名密钥不同，连 30 天 cookie 也无法续命）。扩展现于激活时解析 `DSH_VSCODE_PORT` 作为 `dsh.port` 的默认值（非法值直接报错，绝不静默回落 3080；用户显式 `dsh.port` 设置仍优先），3200 同时落在正式窗口前向扫描（3080–3129，PORT_SCAN_LIMIT=50）之外，双向都不会再顶掉对方。窗口重载或调试会话重启可能携带改动前的环境快照，因此仅设 `DSH_VSCODE_STORAGE_ROOT`（隔离标记本身）也会把默认端口派生到同一值（DEFAULT_PORT+120），任何方式重启的开发宿主都不可能落回 3080 段。
  F5 port isolation: `DSH_VSCODE_PORT` pins the dev host's default port (launch.json now uses 3200). Previously the F5 child shared the default 3080 with installed windows; every takeover left the losing sidebar's iframe holding a stale launch token and a cookie signed by the other home's secret, rendering DSH's 401 body. The extension resolves `DSH_VSCODE_PORT` at activation as the `dsh.port` fallback (invalid values throw instead of silently falling back to 3080; an explicit user `dsh.port` still wins), and 3200 sits beyond the installed windows' forward scan window (3080–3129, PORT_SCAN_LIMIT=50) so neither side can evict the other. Because a window reload or a restarted debug session can inherit a pre-change env snapshot, setting `DSH_VSCODE_STORAGE_ROOT` alone — the isolation marker itself — also derives the default port to the same value (DEFAULT_PORT+120).
- **端口默认值曾被 package.json 注册默认值遮蔽（首轮修复无效的根因）**：`dsh.port` 在 package.json 注册了 `default: 3080`，真实 VS Code 中 `settings.get('port', fallback)` 会返回注册默认值，传入的 fallback 永远不生效——环境变量与派生逻辑都正确执行了，结果却在设置读取层被丢弃（单元测试的假 facade 不模拟注册默认值，因此测试全绿）。读取现改用 `settings.inspect('port')`：仅当用户在任意配置层显式写入 `dsh.port` 时采用该值，否则回落宿主派生默认端口。
  The port default was previously masked by the registered setting default (the reason the first fix round failed in the real host): `dsh.port` is registered in package.json with `default: 3080`, so real `settings.get('port', fallback)` always returned 3080 and the passed fallback was dead code — the env pin and the derivation both ran correctly and were then discarded at the settings layer, while unit-test facades (which do not emulate registered defaults) stayed green. The read now uses `settings.inspect('port')`: an explicit `dsh.port` in any configuration layer still wins, otherwise the host-derived default port applies.

### Fixed / 修复（DSH 0.1.2-rc.1 鉴权启动探测）

- **DSH 已启动却被 F5 等待 30 秒后杀死**：0.1.2-rc.1 的根页面需要启动 token，无鉴权 `GET /` 返回 401；旧探针只接受无鉴权的 `200 + __DSH_BOOT__`，因此把健康子进程误判为未就绪。ServerManager 现在只从本次 owned child 的 per-spawn log 读取 `dsh web: http://127.0.0.1:<expected-port>/?token=...`，严格核对协议、host 与 port 后，以该路径进行认证探测；ready handle 和后续 watchdog 保留认证 URL，但实例注册表不落 token。旧版无鉴权首页仍沿用原探测路径。
  DSH 0.1.2-rc.1 protects the root page with its startup token, so the old unauthenticated `200 + __DSH_BOOT__` probe timed out and killed a healthy child. ServerManager now accepts an authenticated URL only from the current owned child's per-spawn log and only after exact protocol/host/port validation, then uses it for readiness and watchdog probes. The ready handle retains the URL while the registry never persists the token; legacy unauthenticated runtimes keep the original path.

### Technical debt / 技术债（需要彻底重构）

- **运行时边界控制需要一次整体重构，当前隔离仅是明确的止血层**：DSH home/profile 选择、runtime 版本与能力协商、子进程所有权、端口/实例发现、启动 URL 与 token、健康检查以及 installed/dev 多实例协调目前分散在 launch 配置、workspace/globalStorage、`dshHome`、`ServerManager` 和集成插件中，边界由若干隐式约定拼接而成。后续应建立单一、不可变的 runtime identity/boundary 描述，并让 prepare → spawn → authenticated readiness → registry publish → teardown 全链路消费同一身份；在该重构完成前，不应把新增的版本特判或宽松健康判定继续堆入现有路径。
  Runtime boundary management needs a full redesign; the F5 isolation above is an explicit containment measure, not the final architecture. Home/profile selection, version/capability negotiation, child ownership, port/instance discovery, startup URL and token handling, readiness checks, and installed/dev coordination should be derived from one immutable runtime identity across prepare, spawn, authenticated readiness, registry publication, and teardown. Further version special-cases or permissive health heuristics should not be layered onto the current path before that work.

## [1.1.4] - 2026-09-05

### Fixed / 修复（DSH 版本兼容与旧集成包启动失败，2026-09-05）

- **检测 DSH 版本并选择兼容预期**：启动解析器从官方包清单（含 PATH shim 邻近 npm 布局）取得版本；`<0.1.2-rc.1` 标记为 legacy `apiProxy` + native session routes，`>=0.1.2-rc.1` 标记为 `sessionController` + compat routes，未知/fork 标签进入动态协商。版本选择仅用于诊断，集成包以 `ctx.inject` 实际服务探测为最终依据，避免静态依赖在另一条 runtime 线上造成 `pending (waiting for service: apiProxy)` 并让 DSH exit 1。
  DSH version detection now records the expected compatibility line: legacy `apiProxy` with native session routes before `0.1.2-rc.1`, `sessionController` with restored compatibility routes from `0.1.2-rc.1`, and dynamic negotiation for unknown/fork labels. Runtime capability negotiation remains authoritative, preventing a missing optional host service from blocking plugin activation.

### Fixed / 修复（独立 profile 首次启动，2026-09-05）

- **新建 `vscode` profile 会以 YAML 解析错误退出（code=1）**：`profileScaffold` 用合法的顶层空序列 `[]` 初始化 `cordis.patch.yml`，但 spawn 前的 HMR guard 把 `- id: hmr` 直接追加在其后，产生两个顶层 YAML 值。HMR guard 现在识别仅含注释与 `[]` 的空文档并替换该 sentinel；非空 patch 仍保持原有追加、备份和幂等语义。新增真实 scaffold 形态回归测试；本机 DSH `0.1.2-rc.1` 已验证 `--profile vscode --dump-config` exit 0，扩展同款 `--patch ... --profile vscode --no-open` 启动、监听及鉴权 HTTP 200。
  A newly scaffolded `vscode` profile no longer exits with a YAML parse error: the pre-spawn HMR guard now replaces the scaffold's top-level empty-sequence sentinel (`[]`) instead of appending a second top-level YAML value. Non-empty patch files retain the existing append, backup, and idempotency behavior. Covered by the real scaffold shape and verified against DSH `0.1.2-rc.1` through profile parsing and an extension-equivalent authenticated HTTP smoke boot.

### Changed / 变更（独立 profile 启动 + 集成包跨版本兼容层，2026-09-05）

- **`dsh.profile` 默认 `web` → `vscode`（扩展专用 profile）**：扩展拉起的 DSH 子进程与终端 `dsh web`（`web` profile）彻底隔离——独立插件树、独立 cordis patch 层、独立服务实例。背景：web profile 的 cordis.patch.yml 常驻 vscode-integration insert，0.1.2-rc.1 上游移除 `dsh-host-apiproxy`（apiProxy 服务）后，insert 让手动 `dsh web` 也在启动等待中 abort——扩展的事故第一次外溢到终端工作流。DSH 只对内置 profile 名（web/acp/headless/sdk）自动 `initProfile`；新增 `src/profileScaffold.js` 在 `prepareDshHome` 阶段为新默认 profile 写入 `initProfile` 同款三件套（package.json [dsh-base + dsh-web-app]、cordis.patch.yml、pnpm-workspace.yaml；幂等，绝不触碰已初始化的 profile）。默认值此前有四处硬编码（workspaceContext / embedOverlay / runtimeResolver / onboarding），现全部收敛到 `MANAGED_PROFILE` 单一来源。干净重启只作用于扩展自己的子进程；想共用回旧行为设 `"dsh.profile": "web"`（README 双语已备注）。回归测试 `test/unit/profileScaffold.test.js`；受默认值影响的既有断言已同步更新。
  The extension's spawned DSH child now boots a dedicated profile (`vscode` by default, `MANAGED_PROFILE` as the single default source across workspaceContext/embedOverlay/runtimeResolver/onboarding), fully separated from a terminal `dsh web`: own plugin tree, own patch layer, own instance. DSH only auto-initializes built-in profile names; `src/profileScaffold.js` writes an initProfile-equivalent triple (manifest with the web app bundles, empty patch layer, pnpm settings) at prepareDshHome, idempotently and never touching an initialized profile. Set `"dsh.profile": "web"` to share again (noted in both READMEs). Regression tests in `test/unit/profileScaffold.test.js`; default-value assertions across the suite updated to match.
- **`dsh-vscode-integration` 0.8.0 → 0.8.1：跨版本 openPath 兼容层**：静态 `inject` 数组里的 `apiProxy`（0.1.2-rc.1 起上游移除，等价能力移至 `sessionController` 服务、签名从 Typert 信封改为直参 `(path, signal)`）改为运行时 `ctx.inject` 双探测——老 runtime 命中 `apiProxy.host`（信封原样），新 runtime 命中 `sessionController`（桥失败回落原生 opener）；两者皆无只打一条告警、绝不阻塞激活，替代此前"1 entry did not activate"整树起不来的失败模式。回归测试 `runtime-integration/dsh-vscode-integration/test/openPathCompat.test.js`（11 例），并已在本机 npm 0.1.2-rc.1 + 一次性 DSH_HOME 端到端实测：`openPath bridge armed via sessionController`、服务正常起、零 abort。
  The embedded integration plugin no longer statically awaits `apiProxy` (removed upstream in 0.1.2-rc.1; the equivalent lives on the `sessionController` service with a plain `(path, signal)` signature). Both surfaces are claimed dynamically via runtime `ctx.inject` — envelope wrapping on old runtimes, direct-signature wrapping with native-opener fallback on new ones — and a runtime with neither merely logs one warning instead of aborting boot ("1 entry did not activate"). 11 regression tests in `runtime-integration/dsh-vscode-integration/test/openPathCompat.test.js`; verified end-to-end against the real npm 0.1.2-rc.1 runtime with a throwaway DSH_HOME (bridge armed banner, clean boot, zero aborts).

- **`dsh-vscode-integration` 0.8.1 → 0.9.0：session REST 兼容桥（第二轮，同日）**：上游 0.1.2-rc.1 不止移除 apiProxy 服务，还移除了整个 REST 面（`POST /api/session.list|create|rename|prompt`、`GET /api/events.mux`），扩展冻结 wire 客户端（sessionNavigation / dshChatClient / editEventProjector）在 rc.1 上全 404（"DSH 工作区绑定失败 / session API unavailable: HTTP 404"，F5 实测复现）。新增 `lib/compatSessionRoutes.js`：sessionController 存在时经 `ctx.inject(['sessionController','webServer'])` 把五个旧路由挂回 webServer——四个 POST 翻译为 `sessionController.list` / `.commands.create|rename|prompt`（`{client-request}` → `{result:{ok,value}}` 信封逐字段保持；prompt 补 requestId 兜底；host 侧 RemoteError 以抛出表达，桥翻译为 `result.ok:false + error.code` 业务错误），events.mux 以旧 ServerRequest mux 信封转发 `session/event` 总线帧，并每 8s 发一条 keepalive **数据帧**——扩展 SSE 读取器的 15s 停止计时只在数据帧复位，注释行不行。老 runtime 不触发（inject 不命中），原生 apiproxy 路由继续服务。`/api/session.export`（editEventProjector 回扫）暂未桥接：其失败路径本就优雅降级（记日志跳过），留待后续。回归测试 `runtime-integration/dsh-vscode-integration/test/compatSessionRoutes.test.js`（10 例）；本机 npm 0.1.2-rc.1 + 一次性 DSH_HOME 实测：`session.list` → `{items:[]}`、`session.create` → 真 `sessionId`、events.mux → `: connected` + keepalive 帧，全部与冻结协议逐字段一致。
  The rc.1 removal went beyond the apiProxy service — the whole REST surface (`POST /api/session.list|create|rename|prompt`, `GET /api/events.mux`) is gone, so the extension's frozen wire client 404'd on workspace binding (reproduced under F5). `lib/compatSessionRoutes.js` mounts the five removed routes back on the new webServer whenever the sessionController service exists: the four POSTs translate 1:1 to `sessionController.list` / `.commands.{create,rename,prompt}` (envelope preserved field-for-field, requestId defaulting, thrown RemoteErrors mapped to the wire business-error result), and events.mux re-emits the `session/event` bus through the old mux envelope with an 8s keepalive DATA frame (the extension's 15s stall deadline resets only on data frames, not comments). Old runtimes never trigger the inject — their native apiproxy routes keep serving. `/api/session.export` stays unbridged for now (its consumer degrades gracefully by design). 10 regression tests; verified live against rc.1 + a throwaway home: list/create envelopes byte-shape-identical, SSE emits `: connected` + keepalives.

### Fixed / 修复（附件草稿 Ctrl+Enter 误提交，2026-09-04）

- **嵌入侧栏内 Ctrl/Cmd+Enter 把草稿直接提交**：右键"添加到 DSH 对话"后在草稿上写多行回复，按 Ctrl+Enter（macOS ⌘+Enter）想换行——DSH Web 输入框自 2026-08-02 上游提交 `dffe955ed2`（steer 功能）起把该组合键从"换行"改成了"加速提交/steer"，草稿瞬间被发送（用户观感：输入框/草稿消失）。嵌入式集成包 `dsh-vscode-integration`（0.7.0 → 0.8.0）新增 capture 阶段按键桥（仅嵌入 VS Code 时生效）：生产运行时的 Lexical contenteditable 输入框上，把事件实例的 `shiftKey` 遮蔽为 true 后放行——应用的 Shift+Enter 分支与 Lexical 按键映射（shift+enter + 任意 ctrl/meta → 其托管换行命令）用自己的代码完成插入，零 DOM 写入；新版源码的 textarea 输入框则拦截并经 `execCommand('insertText')` → input 事件 → onChange → setDraft 走应用自身草稿记账。空草稿放行——上游"空草稿 Ctrl+Enter steer 整个队列"手势保留；普通 Enter 提交、Shift+Enter 原生换行、输入框外的 Ctrl+Enter、IME 组合态全部不受影响。已在本机真实运行时（npm 0.1.2-rc.1 + 一次性 home）端到端实测：Ctrl+Enter 产生与物理 Shift+Enter 完全一致的 `data-lexical-managed-linebreak` 换行、草稿保留、随后 Enter 正常提交。
  Ctrl/Cmd+Enter in the embedded composer submitted the draft instead of breaking the line: upstream DSH repurposed the chord as accelerated submit/steer in dffe955ed2 (2026-08-02), so writing a multi-line reply over an attachment and pressing Ctrl+Enter (Cmd+Enter on macOS) instantly sent the message. The runtime-integration package (0.7.0 → 0.8.0) restores the chord inside the VS Code embed only: on the released runtimes' Lexical contenteditable it shadows `shiftKey` to true on the keydown instance and lets the event propagate — the app's Shift+Enter branch and Lexical's own key mapping perform the newline through the editor stack; the textarea composer of newer dev builds is claimed and inserted via execCommand's input pipeline. The empty-draft pass-through keeps the upstream queue-steer gesture, and plain Enter / Shift+Enter / chords outside the composer / IME composition are untouched. Verified end-to-end against the real runtime (npm 0.1.2-rc.1, throwaway home): Ctrl+Enter produces the byte-identical managed line break a physical Shift+Enter yields, the draft survives, and Enter still submits.
- 回归测试：`runtime-integration/dsh-vscode-integration/test/ctrlEnterNewline.test.js`（20 例，contenteditable/textarea 双形态：shiftKey 改写放行 / execCommand 插入、双平台组合键、Shift+Enter 与普通 Enter 不动、空草稿放行、composer 外不接管、IME、Alt 变体、execCommand 回退、禁用态、已 preventDefault 尊重）；webviewHandshake 的 keydown 监听 harness 改为数组存储（mac ⌘V 桥测试语义不变）。
...
- **（P0）editObserver 短路 tools/pre-execute waterfall——桥接子进程内所有工具调用（含无工具的 run_code）0ms 报 `Cannot read properties of undefined (reading 'kind')`**：cordis waterfall 监听器不调 `next()` 直接返回会以该返回值短路整条链，dsh-tools 随后读 `gate.kind` 崩溃。监听器现在恒 `return next()`（观测失败也只记日志）；回归测试覆盖 edit/write/run_code/read/空载体/抛错 notify 全路径。事故窗口 2026-09-02 00:19 起（C2 批次文件首次经 dev-host 同步进入 profile）至本修复。
- **(P0) editObserver short-circuited the tools/pre-execute waterfall — every tool call (tool-less run_code included) on bridge-attached extension children failed in 0ms with `Cannot read properties of undefined (reading 'kind')`**: a cordis waterfall listener that returns without calling next() ends the chain with that value, and dsh-tools then read gate.kind of undefined. The listener now always delegates via next() (observation failures only log); regression tests cover edit/write/run_code/read/empty carriers/throwing notify. Incident window: from 2026-09-02 00:19 (the C2 batch first synced into the DSH profile by a dev-host activation) until this fix.
- **（P0）1.0.0 VSIX 捆绑了过期的 runtime-integration 快照**（client.js 缺 dsh_session 跟随桥，KNOWN_ISSUES #9）：装着 1.0.0 的窗口每次激活把回归文件同步进 profile，与 F5 dev-host 互相覆写（junction 乒乓），侧栏丢失会话路由、输入框消失。新增发版门禁：打包树必须与 git HEAD 逐文件一致 + sessionFollow 金丝雀断言，杜绝"脏工作区打包"再次发生。
- **(P0) the 1.0.0 VSIX bundled a stale runtime-integration snapshot** (client.js missing the dsh_session follow bridge, KNOWN_ISSUES #9): windows running 1.0.0 re-synced the regressed files into the profile on every activation, ping-ponging the junction with the F5 dev host; the sidebar lost session routing and the composer disappeared. New packaging gate: the bundled tree must match git HEAD file-for-file plus a sessionFollow canary assertion, so a dirty-workspace package can never ship again.
