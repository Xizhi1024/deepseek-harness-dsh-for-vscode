...
test('extension wires CH1 v2 selection/diagnostics notifications through the coalescer', async () => {
  const fake = createFakeVscode();
  const context = {
    globalStorageUri: { fsPath: path.join(os.tmpdir(), `dsh-extension-notify-${process.pid}`) },
    subscriptions: [],
  };
  const notifyCalls = [];
  const bridge = {
  ...
  for (let i = 0; i < 20; i += 1) {
    fake.selectionHandlerHolder.handler({
      textEditor: { document: { ...document, version: 7 + i } },
    });
  }
  await sleep(200);

  const selectionCalls = notifyCalls.filter((call) => call.method === 'vscode/editor/selectionChanged');
  assert.strictEqual(selectionCalls.length, 1, '20 same-uri selection events must coalesce to one send');
  assert.deepStrictEqual(selectionCalls[0].params, {
    uri,
    version: 26,
    attachmentIds: ['ctx-1'],
  });

  fake.diagnosticsHandlerHolder.handler({
    uris: [
      document.uri,
      { toString: () => 'file:///D:/workspace/unrelated.ts' },
    ],
  });
  await sleep(200);

  const diagnosticCalls = notifyCalls.filter((call) => call.method === 'vscode/diagnosticsChanged');
  assert.strictEqual(diagnosticCalls.length, 1, 'only approved-uri diagnostics are notified');
  assert.deepStrictEqual(diagnosticCalls[0].params, {
    uri,
    attachmentIds: ['ctx-1'],
  });
