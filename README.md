...
> [!IMPORTANT]
> **Starting `dsh web` when VS Code starts is intentional, not a bug.** `dsh.autoStart` defaults to `true`: the extension finds the official npm-installed `@deepseek-ai/dsh`, creates and always uses an extension-owned persistent `.dsh` with the fixed `web` profile. On first launch, official DSH seeds that profile from its bundled `web` template; the extension injects no third-party plugins. You maintain this directory afterward, and the extension keeps reusing it so the sidebar retains its DSH configuration and session state. The default Windows location is `%APPDATA%\Code\User\globalStorage\Xizhi1024.dsh-vs-sidebar\.dsh` (VS Code variants and remote extension hosts may use another global-storage root).
>
> If DSH is not installed yet, the extension still creates its `.dsh`, then tells you to run `npm install -g @deepseek-ai/dsh`; a configuration directory can be created automatically, but the DSH executable cannot. For non-standard npm/Node locations, set `dsh.local.packageRoot` / `dsh.local.nodePath`.
