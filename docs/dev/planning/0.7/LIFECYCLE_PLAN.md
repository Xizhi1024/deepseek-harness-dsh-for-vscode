...
| D | 0.8.0 | **完成（两段）**：D-S1 v3a 协议冻结 31 方法+15 handler `cb751cd`/`4b7bb26`（terminal 同意门/tasks/debug/findFiles/git 只读/editor 元数据+门控 read/progress/statusbar/output/confirm——S0 起 UI 面统一 dsh.bridge.ui 门）；D-remaining `7e90ec0`（DSP Pro 执行、编排者门禁审计集成）：S3 tools.js 31 工具动态注册+断线代际重建、S2a changes/push 审批（ask/session、fail-closed）+changeTracker journal+Undo seam+dsh.changes TreeView、S2b MCP 消费全链（配置合并/${input:}${env:}/同意门/stdio+HTTP/spike PASS）、R23-ext lmRoute provider+R23-dsh /api/lm 桥 token（timingSafeEqual）、Ctrl+K（D8 不绑默认键位）、**L0 硬化**（mcpManager/changeTracker 构造降级+懒校验：VSCODE_MCP_UNAVAILABLE / VSCODE_EDIT_UNAVAILABLE）。编排者门禁拦下 4 缺陷：themeFollow 3 红（L0 生命线被 L2 支撑件炸断）/R25 冻结层断言漏更/secrets 9 处硬编码 token/undo 懒校验缺。**defer E 批**：MCP serve、市场 deep-link、WSL/SSH 远程 smoke（readMcpSources 暂未读 remoteValue 层，随远程 smoke 补）| check:w0 449/0（终值）+ extension-host + **F5 冒烟通过（用户实测）**：三轮真机修复 74a9170（inject 缺 webServer）/b5828f2（v3 notifications 表缺键→initialize 挂死）/9cc7047（ToolRuntime 注册契约），vscode_* 工具注册可用；集成包 node:test 32/0 |tools.js/lmRoute.test 已落地；上收 harness 待 0.8 末（D4） |
